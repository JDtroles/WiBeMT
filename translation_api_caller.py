...
def translate_sentences_via_api(api: str = "Microsoft", dataset_type: str = "Verb_sentences"):
    ...
    :param api: "Microsoft" or "Google" or "DeepL"
    :type api: str
    ...
    for chunk in tqdm(id_plus_sentences_chunked, desc=descr_str):
        ...
        if api == "Microsoft":
            ...
            all_translations.append(deepl_api_caller(chunk))
...
def deepl_api_caller(sentences_to_translate: list) -> list:
    load_dotenv()
    # check if key exists in .env
    key_var_name = 'DEEPL_TRANSLATE_API_KEY'
    if key_var_name not in os.environ:
        raise Exception('Please set/export the environment variable: {}'.format(key_var_name))
    subscription_key = os.environ[key_var_name]

    # check if translation endpoint exists in .env
    endpoint_var_name = 'DEEPL_TRANSLATE_ENDPOINT'
    ...
    # create request data
    data = [('auth_key', subscription_key)]
    ...
    translations = []
    try:
        for text_id, text in zip(ids_sentences, response["translations"]):
            try:
                translations.append([text_id, text['text']])
            except TypeError:
                print("Error in inner layer:")
                print("text_id:", text_id)
                print("text:", text)
                print(response)
                translations.append([text_id, text])
    except TypeError:
        print("Error in outer layer:")
        print(response)
        translations.append([ids_sentences, response])
...
def google_api_caller(sentences_to_translate: list) -> list:
    # create correct format and id list
    ids_sentences = []
    sentences = []
    for sentence in sentences_to_translate:
...
def microsoft_api_caller(sentences_to_translate: list) -> list:
    load_dotenv()
    # create correct format and id list
    dict_sentences = []
    ids_sentences = []
